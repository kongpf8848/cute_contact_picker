#import "CuteContactPickerPlugin.h"
#import <ContactsUI/ContactsUI.h>
#import "ContactModel.h"

NSString*const CHANNEL = @"plugins.flutter.io/cute_contact_picker";
// 跳转原生选择联系人页面
NSString*const METHOD_CALL_NATIVE = @"selectContactNative";
// 获取联系人列表
NSString*const METHOD_CALL_LIST = @"selectContactList";

@interface CuteContactPickerPlugin () <CNContactPickerDelegate>

@property(copy, nonatomic) FlutterResult result;

@end

@implementation CuteContactPickerPlugin{
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  ...
  CuteContactPickerPlugin* instance = [[CuteContactPickerPlugin alloc] initWithViewController:viewController];
...
- (void)contactPicker:(CNContactPickerViewController *)picker didSelectContactProperty:(CNContactProperty *)contactProperty {
    ...
    [_viewController dismissViewControllerAnimated:YES completion:^{
        ...
        // 2. 计算字符串对应的分区索引
        NSInteger section = [collation sectionForObject:name collationStringSelector:@selector(description)];
           
        // 3. 获取该分区对应的首字母
        NSArray *indexTitles = [collation sectionIndexTitles];
        
        if(section>= 0 && section < indexTitles.count) {
            firstLetter= indexTitles[section];
        }else{
            firstLetter= @"#";
        }
        ...
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
                    ...
                    [dict setObject:firstLetter forKey:@"firstLetter"];
                    self.result(dict);
    }];
}
        ...
        if (granted) {
            ...
            [store enumerateContactsWithFetchRequest:request error:nil usingBlock:^(CNContact * _Nonnull contact, BOOL * _Nonnull stop) {
                ...
                if(phoneArray.count > 0){
                    CNLabeledValue *firstLabelValue = phoneArray.firstObject;
                    CNPhoneNumber *number = firstLabelValue.value;
